  ...
  <v-container>
    ...
    <v-container v-if="currentQuestionNumber > 0" :key="currentQuestionNumber">
      ...
      <v-container v-if="currentLocation.children.length > 0">
        <PlanningToolAnswers :key="currentLocation.key + 'planningTool'" :answers="currentLocation.children" @set-current-location="setCurrentLocation" @delete-answer="deleteAnswer" @update-answer="updateAnswer" />
      </v-container>
      <v-container v-if="currentLocation.children.length > 0">
        <PlanningToolAnswersAncestry :answers="currentLocation.children" @set-current-location="setCurrentLocation" />
      </v-container>
      <v-container>
        <h3>Tree View</h3>
        <PlanningToolFullTree :answer="userAnswers[0].answer" :children="userAnswers[0].children" :depth="0" />
      </v-container>
    </v-container>
  </v-container>
</template>
